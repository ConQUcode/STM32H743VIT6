...
// 3508 舵电机光电门归零后累计 ECD 标定值: 左前 ID7, 右前 ID8, 左后 ID5, 右后 ID6
#define CHASSIS_STEER_MOTOR_ECD_RANGE        8192.0f  // 3508 电机本体一圈编码器范围
#define CHASSIS_STEER_WHEEL_TOTAL_ECD_RANGE  (CHASSIS_STEER_MOTOR_ECD_RANGE * 8.0f) // 舵轮输出一圈对应 3508 八圈
#define CHASSIS_STEER_WHEEL_TOTAL_ECD_HALF_RANGE (CHASSIS_STEER_WHEEL_TOTAL_ECD_RANGE / 2.0f) // 舵轮输出半圈对应累计 ECD
#define CHASSIS_STEER_ECD_TO_DEG(ecd_delta)  ((float)(ecd_delta) / CHASSIS_STEER_MOTOR_ECD_RANGE * 360.0f) // 3508 电机 ECD 转电机角度

#define STEERING_CHASSIS_ALIGN_TOTAL_ECD_LF 0         // 左前 ID7 光电门归零后的目标累计 ECD
#define STEERING_CHASSIS_ALIGN_TOTAL_ECD_RF -8455.0f  // 右前 ID8 光电门归零后的目标累计 ECD
#define STEERING_CHASSIS_ALIGN_TOTAL_ECD_LB -2932.0f  // 左后 ID5 光电门归零后的目标累计 ECD
#define STEERING_CHASSIS_ALIGN_TOTAL_ECD_RB 0         // 右后 ID6 光电门归零后的目标累计 ECD

#define STEERING_CHASSIS_ALIGN_ANGLE_LF CHASSIS_STEER_ECD_TO_DEG(STEERING_CHASSIS_ALIGN_TOTAL_ECD_LF) // 左前 ID7 初始电机角度
#define STEERING_CHASSIS_ALIGN_ANGLE_RF CHASSIS_STEER_ECD_TO_DEG(STEERING_CHASSIS_ALIGN_TOTAL_ECD_RF) // 右前 ID8 初始电机角度
#define STEERING_CHASSIS_ALIGN_ANGLE_LB CHASSIS_STEER_ECD_TO_DEG(STEERING_CHASSIS_ALIGN_TOTAL_ECD_LB) // 左后 ID5 初始电机角度
#define STEERING_CHASSIS_ALIGN_ANGLE_RB CHASSIS_STEER_ECD_TO_DEG(STEERING_CHASSIS_ALIGN_TOTAL_ECD_RB) // 右后 ID6 初始电机角度
